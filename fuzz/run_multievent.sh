...
CASES_DIR=$SCRIPT_DIR/cases_multievent
OUT_DIR=$SCRIPT_DIR/out_multievent
LOG_DIR=$SCRIPT_DIR/logs_multievent
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: multievent~n");
        Other ->
...
echo ""
echo "# multievent [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
