#!/usr/bin/env bash
# Phase 11 — runs the statement-pair sequencing fuzz cases generated
# by `gen_stmt_pair.py`. Sibling of run_nested.sh; same per-language
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: stmt-pair~n");
        Other ->
...
echo ""
echo "# stmt-pair [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
