...
# by `gen_state_args.py`. Sibling of run_nested.sh; same per-language
...
CASES_DIR=$SCRIPT_DIR/cases_const_sys
OUT_DIR=$SCRIPT_DIR/out_const_sys
LOG_DIR=$SCRIPT_DIR/logs_const_sys
...
CSPROJ
        ...
        erlang)
            local expected_n
            expected_n=$(grep '^%% FUZZ_EXPECTED_VAL:' "$case_file" | awk '{print $3}')
            local drive_arg
            ...
            if [ -n "$pre_drive_seq" ]; then
                ...
                # list of method names invoked before the verify.
                local seq_body=""
                local IFS=,
                for m in $pre_drive_seq; do
                    seq_body="${seq_body}    _ = ${mod}:${m}(Pid),
"
                done
                unset IFS
                body="${seq_body}    Ret = ${mod}:${verify_method}(Pid),"
...
echo ""
echo "# const-sys [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
