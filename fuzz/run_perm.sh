#!/usr/bin/env bash
# Phase 10 — runs the full-permutation expression fuzz cases generated
# by `gen_perm.py`. Sibling of run_nested.sh; same per-language
...
# delegates to run_nested.sh (which after Phase 10 trim emits only
# p7/p8/p9/p11 — the curated regression patterns).

set -o pipefail

SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
FRAMEC=${FRAMEC:-/Users/marktruluck/projects/framepiler/target/release/framec}
CASES_DIR=$SCRIPT_DIR/cases_perm
OUT_DIR=$SCRIPT_DIR/out_perm
LOG_DIR=$SCRIPT_DIR/logs_perm
...
CSPROJ
        ...
        erlang)
            ...
            local drive_call="${mod}:drive(Pid, ${drive_arg:-6})"
            local body
            if [ "${drive_returns:-yes}" = "yes" ]; then
                body="    Ret = ${drive_call},"
            else
                body="    _ = ${drive_call},
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: perm-fuzz~n");
        Other ->
...
echo ""
echo "# perm-fuzz [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
