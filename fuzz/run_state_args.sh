...
CASES_DIR=$SCRIPT_DIR/cases_state_args
OUT_DIR=$SCRIPT_DIR/out_state_args
LOG_DIR=$SCRIPT_DIR/logs_state_args
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: state-args~n");
        Other ->
...
echo ""
echo "# state-args [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
