...
CASES_DIR=$SCRIPT_DIR/cases_pushpop
OUT_DIR=$SCRIPT_DIR/out_pushpop
LOG_DIR=$SCRIPT_DIR/logs_pushpop
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: pushpop~n");
        Other ->
...
echo ""
echo "# pushpop [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
