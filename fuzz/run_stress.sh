...
CASES_DIR=$SCRIPT_DIR/cases_stress
OUT_DIR=$SCRIPT_DIR/out_stress
LOG_DIR=$SCRIPT_DIR/logs_stress
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: stress~n");
        Other ->
...
echo ""
echo "# stress [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
