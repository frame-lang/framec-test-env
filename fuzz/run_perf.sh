...
# by `gen_perf.py`. Sibling of run_nested.sh; same per-language
...
CASES_DIR=$SCRIPT_DIR/cases_perf
OUT_DIR=$SCRIPT_DIR/out_perf
LOG_DIR=$SCRIPT_DIR/logs_perf
...
# Phase 18 perf wave 2 supports three tiers (smoke/core/full)
# selecting different N values per case (defined in gen_perf.py's
# TIER_N: smoke=100, core=10k, full=1M). The wave-1 inherited
# delegation to run_nested.sh on core is wrong here; perf cases
# encode their own tier in the case_id (perf_<pattern>__<tier>),
# and the smoke-key index filter below handles tier selection
# for our corpus.
...
CSPROJ
        ...
        erlang)
            ...
            local loop_method loop_n loop_kind push_method pop_method pattern_name
            loop_method=$(grep '^%% FUZZ_LOOP_METHOD:' "$case_file" | awk '{print $3}')
            loop_n=$(grep '^%% FUZZ_LOOP_N:' "$case_file" | awk '{print $3}')
            push_method=$(grep '^%% FUZZ_PUSH_METHOD:' "$case_file" | awk '{print $3}')
            pop_method=$(grep '^%% FUZZ_POP_METHOD:' "$case_file" | awk '{print $3}')
            pattern_name=$(grep '^%% FUZZ_PATTERN:' "$case_file" | awk '{print $3}')
            local body
            if [ -n "$loop_method" ]; then
                # Phase 18 perf wave 2: language-native loop with
                # timing. lists:foreach + lists:seq has ~100B per
                # iter overhead in seq construction; for N=1M that
                # allocates 1M ints. Use tail-recursive loop instead
                # to avoid the seq allocation.
                if [ "$loop_method" = "push_pop" ]; then
                    body="    LoopPush = fun L(0) -> ok; L(I) -> ${mod}:${push_method}(Pid), L(I-1) end,
    LoopPop = fun L(0) -> ok; L(I) -> ${mod}:${pop_method}(Pid), L(I-1) end,
    ...
    io:format(\"PERF: pattern=${pattern_name} lang=erlang n=~p ms=~.2f result=~p~n\", [${loop_n}, ElapsedMs, Ret]),"
                else
                    body="    Loop = fun L(0) -> ok; L(I) -> ${mod}:${loop_method}(Pid), L(I-1) end,
    Start = erlang:monotonic_time(microsecond),
    Loop(${loop_n}),
    End = erlang:monotonic_time(microsecond),
    ElapsedMs = (End - Start) / 1000.0,
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: perf~n");
        Other ->
...
echo ""
echo "# perf [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
