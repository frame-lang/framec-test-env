#!/usr/bin/env bash
# Phase 14 — runs the HSM × everything fuzz cases generated
# by `gen_hsm_cross.py`. Sibling of run_nested.sh; same per-language
...
CASES_DIR=$SCRIPT_DIR/cases_hsm_cross
OUT_DIR=$SCRIPT_DIR/out_hsm_cross
LOG_DIR=$SCRIPT_DIR/logs_hsm_cross
...
CSPROJ
        ...
        erlang)
            ...
            # drive_arg is empty for Phase 14 (drive() takes no args).
            # When empty, call drive/1 (just the Pid); otherwise drive/2.
            local drive_call
            if [ -z "$drive_arg" ]; then
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: hsm-cross~n");
        Other ->
...
echo ""
echo "# hsm-cross [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
