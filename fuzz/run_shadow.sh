#!/usr/bin/env bash
# Phase 13 — runs the identifier-shadowing fuzz cases generated
# by `gen_shadow.py`. Sibling of run_nested.sh; same per-language
...
CASES_DIR=$SCRIPT_DIR/cases_shadow
OUT_DIR=$SCRIPT_DIR/out_shadow
LOG_DIR=$SCRIPT_DIR/logs_shadow
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: shadow~n");
        Other ->
...
echo ""
echo "# shadow [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
