...
# by `gen_comments.py`. Sibling of run_nested.sh; same per-language
...
CASES_DIR=$SCRIPT_DIR/cases_comments
OUT_DIR=$SCRIPT_DIR/out_comments
LOG_DIR=$SCRIPT_DIR/logs_comments
...
${body}
    Expected = ${expected_n},
    case Ret of
        Expected -> io:format("PASS: comments~n");
        Other ->
...
echo ""
echo "# comments [$TIER]: $passed / $total passed"
for ln in $LANGS; do
    f="$PER_LANG_LOG_DIR/summary_$ln.tsv"
    [ -f "$f" ] || continue
