#!/bin/bash
# Batched Lua test runner. One lua process loads each test file.
...
OUTPUT="/output"
COMPILE_DIR="/tmp/lua_out"
MANIFEST="/tmp/lua_manifest.tsv"
RUNNER="/opt/TestRunner.lua"
...
lang_tests=$(find /tests/lua -name "*.$ext" 2>/dev/null | sort)
...
if [ "$test_count" -eq 0 ]; then
    echo "TAP version 14"
    echo "1..0 # SKIP no lua tests found"
    exit 0
fi
...
for test_file in $tests; do
    ...
    src=$(ls "$test_src_dir"/*.lua 2>/dev/null | head -1)
    if [ -z "$src" ]; then
...
if [ "$COMPILE_ONLY" = "true" ]; then
    ...
    echo ""
    echo "# lua: $pass passed, $fail failed, $skip skipped"
    exit $fail
fi
