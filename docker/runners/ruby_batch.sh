#!/bin/bash
# Batched Ruby test runner. One long-lived ruby process reads a manifest
# and `load`s each test file in its own toplevel binding. One interpreter
# cold start covers all tests.
...
OUTPUT="/output"
COMPILE_DIR="/tmp/rb_out"
MANIFEST="/tmp/rb_manifest.tsv"
...
lang_tests=$(find /tests/ruby -name "*.$ext" 2>/dev/null | sort)
...
if [ "$test_count" -eq 0 ]; then
    echo "TAP version 14"
    echo "1..0 # SKIP no ruby tests found"
    exit 0
fi
...
for test_file in $tests; do
    ...
    src=$(ls "$test_src_dir"/*.rb 2>/dev/null | head -1)
    if [ -z "$src" ]; then
...
if [ "$COMPILE_ONLY" = "true" ]; then
    ...
    echo ""
    echo "# ruby: $pass passed, $fail failed, $skip skipped"
    exit $fail
fi

# Integrity check — verify dispatcher emitted test_count TAP lines.
tap_out=$(mktemp)
ruby /opt/TestRunner.rb "$MANIFEST" | tee "$tap_out"
rc=${PIPESTATUS[0]:-$?}
