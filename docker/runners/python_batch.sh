#!/bin/bash
# Batched Python test runner. Each .fpy transpiled to .py, then a single
# python3 process (TestRunner.py) imports each module and captures its
# stdio. One interpreter cold start covers the whole run.
...
OUTPUT="/output"
COMPILE_DIR="/tmp/py_out"
MANIFEST="/tmp/py_manifest.tsv"
RUNNER="/opt/TestRunner.py"
...
lang_tests=$(find /tests/python -name "*.$ext" 2>/dev/null | sort)
...
for test_file in $tests; do
    ...
    src=$(ls "$test_src_dir"/*.py 2>/dev/null | head -1)
    if [ -z "$src" ]; then
...
if [ "$COMPILE_ONLY" = "true" ]; then
    ...
    echo ""
    echo "# python: $pass passed, $fail failed, $skip skipped"
    exit $fail
fi

# Integrity check — verify dispatcher emitted test_count TAP lines.
tap_out=$(mktemp)
python3 "$RUNNER" "$MANIFEST" | tee "$tap_out"
rc=${PIPESTATUS[0]:-$?}
