...
impl Clone for StateParamsFrameEvent {
    fn clone(&self) -> Self {
        Self {
...
let mut __e = StateParamsFrameEvent::new("start");
let __ctx = StateParamsFrameContext::new(__e.clone(), None);
...
let mut __e = StateParamsFrameEvent::new("get_value");
let __ctx = StateParamsFrameContext::new(__e.clone(), None);
self._context_stack.push(__ctx);
match self.__compartment.state.as_str() {
...
self.__transition(StateParamsCompartment::new("Counter"));
    ...
    fn _s_Idle_get_value(&mut self, __e: &StateParamsFrameEvent) {
if let Some(ctx) = self._context_stack.last_mut() { ctx._return = Some(Box::new(0)); }
return;
    }

    fn _s_Counter_get_value(&mut self, __e: &StateParamsFrameEvent) {
