...
func (s *S) _state_A(__e *SFrameEvent) {
    if __e._message == "E" {
        y := 0
        s._state_P(__e); y = 1  // inline native after forward
        s._state_stack = append(s._state_stack, s.__compartment.copy()); y = 2  // inline native after stack push
        _ = y
...
func main() {
	...
	s := NewS()
	s.E()
	fmt.Println("ok 1 - mixed_inline_forward_stack_then_transition_exec")
}
