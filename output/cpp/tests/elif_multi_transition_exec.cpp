    ...
    void _state_A(SFrameEvent& __e) {
        if (__e._message == "e") {
            if (false) {
                ...
                auto __new_compartment = std::make_unique<SCompartment>("C");
                __new_compartment->parent_compartment = __compartment->clone();
                __transition(std::move(__new_compartment));
                return;
            } else {
                auto __new_compartment = std::make_unique<SCompartment>("D");
                __new_compartment->parent_compartment = __compartment->clone();
                __transition(std::move(__new_compartment));
                return;
            }
...
int main() {
    ...
    try {
        S s;
        s.e();
        printf("ok 1 - elif_multi_transition_exec\n");
    } catch (...) {
        printf("not ok 1 - elif_multi_transition_exec\n");
    }
    return 0;
}
