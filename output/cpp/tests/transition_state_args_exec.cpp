...
private:
    ...
    void _state_A(SFrameEvent& __e) {
        if (__e._message == "e") {
            ...
            __new_compartment->state_args["0"] = std::any(1);
            __new_compartment->state_args["1"] = std::any(2);
            __new_compartment->state_args["2"] = std::any(3);
...
int main() {
    ...
    try {
        S s;
        s.e();
        printf("ok 1 - transition_state_args_exec\n");
    } catch (...) {
        printf("not ok 1 - transition_state_args_exec\n");
    }
    return 0;
}
