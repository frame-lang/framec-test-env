    ...
    private void _state_A(SFrameEvent __e) {
        if (__e._message.equals("e")) {
            if (false) {
                ...
                var __compartment = new SCompartment("C");
                __compartment.parent_compartment = this.__compartment.copy();
                __transition(__compartment);
                return;
            } else {
                var __compartment = new SCompartment("D");
                __compartment.parent_compartment = this.__compartment.copy();
                __transition(__compartment);
                return;
            }
...
class Main {
    public static void main(String[] args) {
        ...
        try {
            S s = new S();
            s.e();
            System.out.println("ok 1 - elif_multi_transition_exec");
        } catch (Exception ex) {
            System.out.println("not ok 1 - elif_multi_transition_exec # " + ex);
        }
    }
}
