...
class S {
    ...
    private void _state_A(SFrameEvent __e) {
        if (__e._message.equals("e")) {
            int y = 0;
            _state_P(__e); y = 1;  // inline native after forward
            _state_stack.add(__compartment.copy()); y = 2;  // inline native after stack push
...
class Main {
    public static void main(String[] args) {
        ...
        try {
            S s = new S();
            s.e();
            System.out.println("ok 1 - mixed_inline_forward_stack_then_transition_exec");
        } catch (Exception ex) {
            System.out.println("not ok 1 - mixed_inline_forward_stack_then_transition_exec # " + ex);
        }
    }
}
