...
class ContextDataTestFrameContext {
    public event: ContextDataTestFrameEvent;
    public _return: any;
    public _data: Record<string, any>;

    constructor(event: ContextDataTestFrameEvent, default_return: any) {
        this.event = event;
...
class ContextDataTestCompartment {
    ...
    public parent_compartment: ContextDataTestCompartment | null;

    constructor(state: string, parent_compartment: ContextDataTestCompartment | null = null) {
        this.state = state;
        this.state_args = {  };
        this.state_vars = {  };
        this.enter_args = {  };
    ...
    public copy(): ContextDataTestCompartment {
...
class ContextDataTest {
    ...
    private __compartment: ContextDataTestCompartment;
    private __next_compartment: ContextDataTestCompartment | null;
    ...
    private __kernel(__e: ContextDataTestFrameEvent) {
    ...
    private __router(__e: ContextDataTestFrameEvent) {
    ...
    private __transition(next_compartment: ContextDataTestCompartment) {
        this.__next_compartment = next_compartment;
    }

    public process_with_data(value: number): string {
    ...
    public check_data_isolation(): string {
    ...
    public transition_preserves_data(x: number): string {
    ...
    private _state_Start(__e: ContextDataTestFrameEvent) {
        if (__e._message === "<$") {
            // Exit handler can access data set by event handler
            (this._context_stack[this._context_stack.length - 1]._data["trace"] as string[]).push("exit");
    ...
    private _state_End(__e: ContextDataTestFrameEvent) {
        if (__e._message === "$>") {
            // Enter handler can access data set by previous handlers
            (this._context_stack[this._context_stack.length - 1]._data["trace"] as string[]).push("enter");
            this._context_stack[this._context_stack.length - 1]._data["ended_in"] = "End";

            // Build final result from accumulated data
            const trace = this._context_stack[this._context_stack.length - 1]._data["trace"] as string[];
