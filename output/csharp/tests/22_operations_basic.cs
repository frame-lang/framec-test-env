using System;
using System.Collections.Generic;


using System;
using System.Collections.Generic;

class OperationsTestFrameEvent {
    public string _message;
    public Dictionary<string, object> _parameters;

    public OperationsTestFrameEvent(string message) {
        this._message = message;
        this._parameters = null;
    }

    public OperationsTestFrameEvent(string message, Dictionary<string, object> parameters) {
        this._message = message;
        this._parameters = parameters;
    }
}

class OperationsTestFrameContext {
    public OperationsTestFrameEvent _event;
    public object _return;
    public Dictionary<string, object> _data;

    public OperationsTestFrameContext(OperationsTestFrameEvent ev, object defaultReturn) {
        this._event = ev;
        this._return = defaultReturn;
        this._data = new Dictionary<string, object>();
    }
}

class OperationsTestCompartment {
    public string state;
    public Dictionary<string, object> state_args;
    public Dictionary<string, object> state_vars;
    public Dictionary<string, object> enter_args;
    public Dictionary<string, object> exit_args;
    public OperationsTestFrameEvent forward_event;
    public OperationsTestCompartment parent_compartment;

    public OperationsTestCompartment(string state) {
        this.state = state;
        this.state_args = new Dictionary<string, object>();
        this.state_vars = new Dictionary<string, object>();
        this.enter_args = new Dictionary<string, object>();
        this.exit_args = new Dictionary<string, object>();
        this.forward_event = null;
        this.parent_compartment = null;
    }

    public OperationsTestCompartment Copy() {
        OperationsTestCompartment c = new OperationsTestCompartment(this.state);
        c.state_args = new Dictionary<string, object>(this.state_args);
        c.state_vars = new Dictionary<string, object>(this.state_vars);
        c.enter_args = new Dictionary<string, object>(this.enter_args);
        c.exit_args = new Dictionary<string, object>(this.exit_args);
        c.forward_event = this.forward_event;
        c.parent_compartment = this.parent_compartment;
        return c;
    }
}

class OperationsTest {
    private List<OperationsTestCompartment> _state_stack;
    private OperationsTestCompartment __compartment;
    private OperationsTestCompartment __next_compartment;
    private List<OperationsTestFrameContext> _context_stack;
    public int last_result = 0;

    public OperationsTest() {
        _state_stack = new List<OperationsTestCompartment>();
        _context_stack = new List<OperationsTestFrameContext>();
        __compartment = new OperationsTestCompartment("Ready");
        __next_compartment = null;
        OperationsTestFrameEvent __frame_event = new OperationsTestFrameEvent("$>");
        OperationsTestFrameContext __ctx = new OperationsTestFrameContext(__frame_event, null);
        _context_stack.Add(__ctx);
        __kernel(_context_stack[_context_stack.Count - 1]._event);
        _context_stack.RemoveAt(_context_stack.Count - 1);
    }

    private void __kernel(OperationsTestFrameEvent __e) {
        __router(__e);
        while (__next_compartment != null) {
            var next_compartment = __next_compartment;
            __next_compartment = null;
            OperationsTestFrameEvent exit_event = new OperationsTestFrameEvent("<$");
            __router(exit_event);
            __compartment = next_compartment;
            if (__compartment.forward_event == null) {
                OperationsTestFrameEvent enter_event = new OperationsTestFrameEvent("$>");
                __router(enter_event);
            } else {
                var forward_event = __compartment.forward_event;
                __compartment.forward_event = null;
                if (forward_event._message == "$>") {
                    __router(forward_event);
                } else {
                    OperationsTestFrameEvent enter_event = new OperationsTestFrameEvent("$>");
                    __router(enter_event);
                    __router(forward_event);
                }
            }
        }
    }

    private void __router(OperationsTestFrameEvent __e) {
        string state_name = __compartment.state;
        if (state_name == "Ready") {
            _state_Ready(__e);
        }
    }

    private void __transition(OperationsTestCompartment next) {
        __next_compartment = next;
    }

    public int compute(int a, int b) {
        Dictionary<string, object> __params = new Dictionary<string, object>();
        __params["a"] = a;
        __params["b"] = b;
        OperationsTestFrameEvent __e = new OperationsTestFrameEvent("compute", __params);
        OperationsTestFrameContext __ctx = new OperationsTestFrameContext(__e, null);
        _context_stack.Add(__ctx);
        __kernel(_context_stack[_context_stack.Count - 1]._event);
        var __result = (int) _context_stack[_context_stack.Count - 1]._return;
        _context_stack.RemoveAt(_context_stack.Count - 1);
        return __result;
    }

    public int get_last_result() {
        OperationsTestFrameEvent __e = new OperationsTestFrameEvent("get_last_result");
        OperationsTestFrameContext __ctx = new OperationsTestFrameContext(__e, null);
        _context_stack.Add(__ctx);
        __kernel(_context_stack[_context_stack.Count - 1]._event);
        var __result = (int) _context_stack[_context_stack.Count - 1]._return;
        _context_stack.RemoveAt(_context_stack.Count - 1);
        return __result;
    }

    private void _state_Ready(OperationsTestFrameEvent __e) {
        if (__e._message == "compute") {
            var a = (int) __e._parameters["a"];
            var b = (int) __e._parameters["b"];
            // Use instance operations
            int sum_val = this.add(a, b);
            int prod_val = this.multiply(a, b);
            int last_result = sum_val + prod_val;
            _context_stack[_context_stack.Count - 1]._return = last_result;
            return;
        } else if (__e._message == "get_last_result") {
            _context_stack[_context_stack.Count - 1]._return = this.last_result;
            return;
        }
    }

    public int add(int x, int y) {
                    return x + y;
    }

    public int multiply(int x, int y) {
                    return x * y;
    }

    public static int factorial(int n) {
                    if (n <= 1) {
                        return 1;
                    }
                    return n * OperationsTest.factorial(n - 1);
    }

    public static bool is_even(int n) {
                    return n % 2 == 0;
    }
}

class Program {
    static void Main(string[] args) {
        Console.WriteLine("=== Test 22: Operations Basic (C#) ===");
        var s = new OperationsTest();

        // Test 1: Instance operations
        var result = (int)s.add(3, 4);
        if (result != 7) {
            Console.WriteLine("FAIL: Expected 7, got " + result);
            Environment.Exit(1);
        }
        Console.WriteLine("1. add(3, 4) = " + result);

        result = (int)s.multiply(3, 4);
        if (result != 12) {
            Console.WriteLine("FAIL: Expected 12, got " + result);
            Environment.Exit(1);
        }
        Console.WriteLine("2. multiply(3, 4) = " + result);

        // Test 2: Operations used in handler
        result = (int)s.compute(3, 4);
        // compute returns add(3,4) + multiply(3,4) = 7 + 12 = 19
        if (result != 19) {
            Console.WriteLine("FAIL: Expected 19, got " + result);
            Environment.Exit(1);
        }
        Console.WriteLine("3. compute(3, 4) = " + result);

        // Test 3: Static operations
        result = OperationsTest.factorial(5);
        if (result != 120) {
            Console.WriteLine("FAIL: Expected 120, got " + result);
            Environment.Exit(1);
        }
        Console.WriteLine("4. factorial(5) = " + result);

        var is_even = OperationsTest.is_even(4);
        if (is_even != true) {
            Console.WriteLine("FAIL: Expected true, got " + is_even);
            Environment.Exit(1);
        }
        Console.WriteLine("5. is_even(4) = " + is_even);

        is_even = OperationsTest.is_even(7);
        if (is_even != false) {
            Console.WriteLine("FAIL: Expected false, got " + is_even);
            Environment.Exit(1);
        }
        Console.WriteLine("6. is_even(7) = " + is_even);

        // Test 4: Static via class
        result = OperationsTest.factorial(4);
        if (result != 24) {
            Console.WriteLine("FAIL: Expected 24, got " + result);
            Environment.Exit(1);
        }
        Console.WriteLine("7. OperationsTest.factorial(4) = " + result);

        Console.WriteLine("PASS: Operations basic works correctly");
    }
}
