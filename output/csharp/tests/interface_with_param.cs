...
class S {
    ...
    public void ev(int n) {
        Dictionary<string, object> __params = new Dictionary<string, object>();
        __params["n"] = n;
        SFrameEvent __e = new SFrameEvent("ev", __params);
...
class Program {
    static void Main(string[] args) {
        ...
        try {
            S s = new S();
            Console.WriteLine("ok 1 - interface_with_param");
        } catch (Exception ex) {
            Console.WriteLine("not ok 1 - interface_with_param # " + ex);
        }
    }
}
