...
class S {
    ...
    private void _state_A(SFrameEvent __e) {
        if (__e._message == "e") {
            ...
            __new_compartment.state_args["0"] = 1;
            __new_compartment.state_args["1"] = 2;
            __new_compartment.state_args["2"] = 3;
...
class Program {
    static void Main(string[] args) {
        ...
        try {
            S s = new S();
            s.e();
            Console.WriteLine("ok 1 - transition_state_args_exec");
        } catch (Exception ex) {
            Console.WriteLine("not ok 1 - transition_state_args_exec # " + ex);
        }
    }
}
