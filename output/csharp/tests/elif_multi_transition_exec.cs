    ...
    private void _state_A(SFrameEvent __e) {
        if (__e._message == "e") {
            if (false) {
                ...
                { var __new_compartment = new SCompartment("C");
                __new_compartment.parent_compartment = __compartment.Copy();
                __transition(__new_compartment); }
                return;
            } else {
                { var __new_compartment = new SCompartment("D");
                __new_compartment.parent_compartment = __compartment.Copy();
                __transition(__new_compartment); }
                return;
            }
...
class Program {
    static void Main(string[] args) {
        ...
        try {
            S s = new S();
            s.e();
            Console.WriteLine("ok 1 - elif_multi_transition_exec");
        } catch (Exception ex) {
            Console.WriteLine("not ok 1 - elif_multi_transition_exec # " + ex);
        }
    }
}
