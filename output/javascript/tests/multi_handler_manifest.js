...
export class S {
    ...
    g(x) {
        const __e = new SFrameEvent("g", {"x": x});
    ...
    e(p, q) {
        const __e = new SFrameEvent("e", {"p": p, "q": q});
        const __ctx = new SFrameContext(__e, null);
        this._context_stack.push(__ctx);
        this.__kernel(__e);
        this._context_stack.pop();
    ...
    _state_A(__e) {
        if (__e._message === "e") {
            const p = __e._parameters?.["p"];
            const q = __e._parameters?.["q"];
...
function main() {
    ...
    try {
        ...
        console.log("ok 1 - multi_handler_manifest");
    } catch (ex) {
        console.log(`not ok 1 - multi_handler_manifest # ${ex}`);
    }
}
main();
