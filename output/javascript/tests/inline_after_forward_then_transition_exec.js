...
export class S {
    ...
    _state_A(__e) {
        if (__e._message === "e") {
            this._state_P(__e);; let a = 1; let b = 2; // multiple natives after forward on same line
...
function main() {
    ...
    try {
        ...
        console.log("ok 1 - inline_after_forward_then_transition_exec");
    } catch (ex) {
        console.log(`not ok 1 - inline_after_forward_then_transition_exec # ${ex}`);
    }
}
main();
