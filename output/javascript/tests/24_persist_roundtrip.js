...
export class PersistRoundtripFrameEvent {
    _message;
    _parameters;
...
export class PersistRoundtripFrameContext {
    event;
    _return;
    _data;
...
export class PersistRoundtripCompartment {
    state;
    state_args;
    state_vars;
    enter_args;
...
export class PersistRoundtrip {
    ...
    static restoreState(json) {
        const deserializeComp = (data) => {
            if (!data) return null;
            const comp = new PersistRoundtripCompartment(data.state);
...
function main() {
    console.log("=== Test 24: Persist Roundtrip (JavaScript) ===");
