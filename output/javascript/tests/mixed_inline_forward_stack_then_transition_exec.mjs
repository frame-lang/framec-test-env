...
export class S {
    ...
    _state_A(__e) {
        if (__e._message === "e") {
            let y = 0
            this._state_P(__e);; y = 1  // inline native after forward
            this._state_stack.push(this.__compartment.copy());; y = 2  // inline native after stack push
...
function main() {
    ...
    try {
        ...
        console.log("ok 1 - mixed_inline_forward_stack_then_transition_exec");
    } catch (ex) {
        console.log(`not ok 1 - mixed_inline_forward_stack_then_transition_exec # ${ex}`);
    }
}
main();
