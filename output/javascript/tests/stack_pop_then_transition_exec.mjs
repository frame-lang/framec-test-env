...
export class S {
    ...
    _state_A(__e) {
        if (__e._message === "e") {
            this._state_stack.push(this.__compartment.copy());// Push current state to stack
...
function main() {
    ...
    try {
        ...
        console.log("ok 1 - stack_pop_then_transition_exec");
    } catch (ex) {
        console.log(`not ok 1 - stack_pop_then_transition_exec # ${ex}`);
    }
}
main();
