    ...
    _state_A(__e) {
        if (__e._message === "e") {
            if (false) {
                ...
                const __compartment = new SCompartment("C", this.__compartment.copy());
                this.__transition(__compartment);
                return;
            } else {
                const __compartment = new SCompartment("D", this.__compartment.copy());
                this.__transition(__compartment);
                return;
            }
        }
...
function main() {
    ...
    try {
        ...
        console.log("ok 1 - elif_multi_transition_exec");
    } catch (ex) {
        console.log(`not ok 1 - elif_multi_transition_exec # ${ex}`);
    }
}
main();
