...
const FRM = path.join('sandbox','sim','AdapterStdioPauseStep.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_pause_step.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_pause_step.js');
...
function run() {
  ...
  child.on('exit', (code) => {
    ...
    const pass = lines[0] === init && lines.includes(pauseCmd) && lines.includes(nextCmd);
    if (pass) console.log('PASS adapter_pause_then_step');
    else {
      console.error('FAIL adapter_pause_then_step');
