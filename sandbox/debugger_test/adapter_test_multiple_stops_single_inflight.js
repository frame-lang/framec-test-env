...
const FRM = path.join('sandbox','sim','AdapterStdioMultipleStops.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_multistops.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_multistops.js');
...
function run() {
  ...
  // Feed two breakpoint stops in a row; expect only one 'next'
  const seq = [
  ...
  child.on('exit', (code) => {
    ...
    const pass = lines[0] === init && nextCount === 1;
    if (pass) console.log('PASS adapter_multiple_stops_single_inflight');
    else {
      console.error('FAIL adapter_multiple_stops_single_inflight');
