...
const FRM = path.join('sandbox','sim','AdapterStdioStepIn.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_stepin.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_stepin.js');
...
function run() {
  ...
  child.on('exit', (code) => {
    ...
    const pass = lines[0] === init && lines.includes(stepIn);
    if (pass) console.log('PASS adapter_stepIn_single_inflight');
    else {
      console.error('FAIL adapter_stepIn_single_inflight');
