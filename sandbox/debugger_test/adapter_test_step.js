...
const FRM = path.join('sandbox','sim','AdapterStdioStep.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_step.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_step.js');
...
function run() {
  ...
  // Feed: connected -> stopped(breakpoint) -> continued -> stopped(step) -> terminated
  const events = [
  ...
  child.on('exit', (code) => {
    ...
    const numNext = lines.filter(l => l === nextCmd).length;
    const pass = lines[0] === init && numNext === 1;
    if (pass) console.log('PASS adapter_step_single_inflight');
    else {
      console.error('FAIL adapter_step_single_inflight');
