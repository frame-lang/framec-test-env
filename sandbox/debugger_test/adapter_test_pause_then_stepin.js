...
const FRM = path.join('sandbox','sim','AdapterStdioPauseStepIn.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_pause_stepin.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_pause_stepin.js');
...
function run() {
  ...
  const seq = [
    ...
    { type: 'event', event: 'stopped', data: { reason: 'pause' } },
  ...
  child.on('exit', (code) => {
    ...
    const pauseCmd = JSON.stringify({ type: 'command', action: 'pause' });
    const stepIn = JSON.stringify({ type: 'command', action: 'stepIn' });
    const pass = lines[0] === init && lines.includes(pauseCmd) && lines.includes(stepIn);
    if (pass) console.log('PASS adapter_pause_then_stepIn');
    else {
      console.error('FAIL adapter_pause_then_stepIn');
