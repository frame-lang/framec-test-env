...
const FRM = path.join('sandbox','sim','AdapterStdioOutput.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_output.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_output.js');
...
function run() {
  ...
  const seq = [
    ...
    { type: 'event', event: 'output', data: { text: 'Hello', stream: 'stdout' } },
  ...
  child.on('exit', (code) => {
    ...
    const expectedOutput = JSON.stringify({ type: 'event', event: 'output', data: { text: 'Hello', stream: 'stdout' } });
    const pass = lines[0] === expectedInit && lines.includes(expectedOutput);
    if (pass) console.log('PASS adapter_output_passthrough');
    else {
      console.error('FAIL adapter_output_passthrough');
