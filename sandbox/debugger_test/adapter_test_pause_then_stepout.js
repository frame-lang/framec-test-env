...
const FRM = path.join('sandbox','sim','AdapterStdioPauseStepOut.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_pause_stepout.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_pause_stepout.js');
...
function run() {
  ...
  child.on('exit', (code) => {
    ...
    const pass = lines[0] === init && lines.includes(pauseCmd) && lines.includes(stepOut);
    if (pass) console.log('PASS adapter_pause_then_stepOut');
    else {
      console.error('FAIL adapter_pause_then_stepOut');
