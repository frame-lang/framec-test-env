...
const FRM = path.join('sandbox','sim','AdapterStdioPause.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_pause.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_pause.js');
...
function run() {
  ...
  child.on('exit', (code) => {
    ...
    const pass = lines[0] === init && lines.includes(pauseCmd) && lines.includes(cont);
    if (pass) console.log('PASS adapter_pause_resume');
    else {
      console.error('FAIL adapter_pause_resume');
