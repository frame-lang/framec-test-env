...
const FRM = path.join('sandbox','sim','AdapterStdioException.frm');
const OUT_TS = path.join('sandbox','sim','gen','adapter_exception.ts');
const OUT_JS = path.join('sandbox','sim','gen_js','sandbox','sim','gen','adapter_exception.js');
...
function run() {
  ...
  const seq = [
    ...
    { type: 'event', event: 'stopped', data: { reason: 'exception', message: 'boom' } },
  ...
  child.on('exit', (code) => {
    ...
    const hasCont = lines.includes(cont);
    if (hasInit && hasCont) console.log('PASS adapter_exception_continue');
    else {
      console.error('FAIL adapter_exception_continue');
