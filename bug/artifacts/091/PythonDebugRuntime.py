...
class PythonDebugRuntime:
    def __init__(self, *sys_params):
        ...
        state_args = {}
        self._compartment = FrameCompartment("__PythonDebugRuntime_state_Connecting", enter_args=enter_args, state_args=state_args)
        self._stack = []
    ...
    def _action_traceDispatch(self, frame, event, arg):
        ...
        if event == "line":
                self.dispatchAsync(system.onTraceLine(frame, frame.f_lineno))
        elif event == "call":
                self.dispatchAsync(system.onTraceCall(frame))
        elif event == "return":
                self.dispatchAsync(system.onTraceReturn(frame))
        elif event == "exception":
                self.dispatchAsync(system.onTraceException(frame, arg[0], arg[1]))
    ...
    def _action_sendMessage(self, message):
        ...
        self.dispatchAsync(self.sendSocketData(data))
    ...
    def _action_handleMessage(self, message):
        ...
        if message.type == "command":
                ...
                if action == "initialize":
                        ...
                        self.dispatchAsync(system.onContinueExecution())
                elif action == "next":
                        self.dispatchAsync(system.onNextStep())
                elif action == "stepIn":
                        self.dispatchAsync(system.onStepInto())
                elif action == "stepOut":
                        self.dispatchAsync(system.onStepOutOf())
                elif action == "setBreakpoints":
                        self.dispatchAsync(system.onSetBreakpoints(message.data.lines))
                elif action == "pause":
                        self.dispatchAsync(system.onPause())
                elif action == "terminate":
                        self.dispatchAsync(system.onTerminate())
                elif action == "getCallStack":
        # Phase 7: Call stack generation
                        threadId = message.data.get("threadId", 1)
                        self.dispatchAsync(system.onGetCallStack(threadId))
    ...
    def _action_applyInitializationConfig(self, data):
        ...
        self.dispatchAsync(system.onInitialize(config))
    ...
    def _event_onInitialize(self, __e: FrameEvent, compartment: FrameCompartment):
        ...
        if c.state == "__PythonDebugRuntime_state_AwaitingInit":
            ...
            if self.stopOnEntry: self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_WaitingForEntry")); return
            else: self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Running")); return
    ...
    def _event_onTerminate(self, __e: FrameEvent, compartment: FrameCompartment):
        ...
        if c.state == "__PythonDebugRuntime_state_Running":
            ...
            self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Terminating"))
            return
    ...
    def _event_onTraceException(self, __e: FrameEvent, compartment: FrameCompartment):
        ...
        if c.state == "__PythonDebugRuntime_state_Running":
            ...
            if self.stopOnException and self.isUserCode(frame): self.pauseExecution(frame, "exception", frameLine); self.enterPausedState(); self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Paused")); return
    ...
    def _event_onTraceLine(self, __e: FrameEvent, compartment: FrameCompartment):
        ...
        if c.state == "__PythonDebugRuntime_state_WaitingForEntry":
            ...
            if frameLine is not None and self.isUserCode(frame): self.pauseExecution(frame, "entry", frameLine); self.enterPausedState(); self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Paused")); return
            ...
            if frameLine in self.breakpoints and self.isUserCode(frame): self.pauseExecution(frame, "breakpoint", frameLine); self.enterPausedState(); self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Paused")); return
            
            # Handle step modes
            if self.stepMode == "stepIn":
                if frameLine is not None and self.isUserCode(frame): self.pauseExecution(frame, "step", frameLine); self.enterPausedState(); self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Paused")); return
            elif self.stepMode == "next":
                if self.shouldStopStepNext(frame, frameLine): self.pauseExecution(frame, "step", frameLine); self.enterPausedState(); self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Paused")); return
            elif self.stepMode == "stepOut":
                # Will be handled in onTraceReturn
                pass
            
            # Handle pause request
            if self.pauseRequested and frameLine is not None and self.isUserCode(frame): self.pauseExecution(frame, "pause", frameLine); self.enterPausedState(); self._frame_transition(FrameCompartment("__PythonDebugRuntime_state_Paused")); return
