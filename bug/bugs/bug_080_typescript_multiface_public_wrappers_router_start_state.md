...
2) Run the shared env smoke (generates TS, compiles, and runs harness):
...
3) Inspect generated TS in the OUT directory if needed; harness asserts wrappers+router semantics and prints `ADAPTER_SMOKE_OK` on success.
...
- 2025-11-20: Verified wrappers+router+start state in shared env on v0.86.54 (`run_adapter_smoke.sh`); closed — vscode_editor
