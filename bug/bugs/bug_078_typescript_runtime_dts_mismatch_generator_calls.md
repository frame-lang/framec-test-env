...
## Reproduction / Validation (shared env)
1) Ensure `framec --version` prints the target version (e.g., v0.86.49+).
2) Use the shared env adapter smoke (generates TS, compiles via local tsconfig with path mapping to in-tree d.ts, and runs a Node harness):
...
3) Expected from v0.86.49+: no TS signature/property errors; harness prints `ADAPTER_SMOKE_OK` (confirmed on v0.86.54).
...
- Implemented an explicit `frame_runtime_ts/index.d.ts` in the workspace and used tsconfig path mapping in the shared env harness to match the actual runtime implementation in `frame_runtime_ts/index.ts`:
