...
const drain = (): any[] => {
  const q = Array.isArray((fsm as any).commandQueue)
    ? (fsm as any).commandQueue.slice()
    : [];
  (fsm as any).commandQueue = [];
  return q;
};
...
cmds = drain();
const acts = cmds.map((c: any) => c && c.action);
...
fsm.enqueueCommand("continue", {});
cmds = drain();
assert(cmds.filter((c: any) => c && c.action === "continue").length === 1, "expected single continue enqueued after ready");
